...
describe('Testing Birthdays', () => {
  it('Valid Birthdays', () => {
    validBirthdays.forEach((birthday) =>
      expect(isValidFnrBirthdate(birthday)).toBeTruthy()
    )
  })

  it('Invalid Birthdays', () => {
    invalidBirthdays.forEach((birthday) =>
      expect(isValidFnrBirthdate(birthday)).toBeFalsy()
    )
  })
})
...
describe('Validate and Create Birthdays', () => {
  it('#validateAndCreateBirthday February date too high, leap year', () => {
    expect(validateAndCreateBirthday(30, 2, 88)).toEqual('290288')
  })
  it('#validateAndCreateBirthday, February date too high, not leap year', () => {
    expect(validateAndCreateBirthday(30, 2, 89)).toEqual('280289')
  })
})
...
describe('Test Leap Years', () => {
  it('Valid Leap Years', () => {
    ;[
      ...
      76, 80, 84, 88, 92, 96,
    ].forEach((year) => expect(isLeapYear(year)).toBeTruthy())
  })
